...
- Android release packet schema validation is available via `npm run android:release:packet:check` and is enforced before artifact upload in CI, including per-document SHA-256 integrity verification, source git revision validation, CI expected-SHA binding, CI run metadata validation when present, CI expected run-id/attempt binding, and CI workflow/run-number binding.
