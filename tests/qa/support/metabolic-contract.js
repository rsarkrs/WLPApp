...
const { activityMultipliers, roundingPolicy } = fixtures.meta;
...
function computeBmrTdeeContract(input) {
  const { sex, weightKg, heightCm, ageYears, activityLevel } = input;
  ...
  return {
    ...
    activityMultiplier,
    roundingPolicy: roundingPolicy.id,
  };
}

function applyWeeklyCapContract({ currentWeightKg, requestedWeeklyLossKg }) {
...
function applyCalorieFloorContract({ sex, proposedDailyCalories }) {
  assertSupportedSex(sex);
...
function allocateMacrosContract({ dailyCalories, weightKg }) {
  if (dailyCalories <= 0) {
  ...
  const fallbackResidualCalories =
    dailyCalories - (fallbackProteinG * 4 + fallbackFatG * 9 + fallbackCarbG * 4);
  ...
  return {
    ...
    residualCalories: fallbackResidualCalories,
  };
}

const contextToFunction = {
  bmr_tdee: computeBmrTdeeContract,
  weekly_cap: applyWeeklyCapContract,
  calorie_floor: applyCalorieFloorContract,
  macro_allocation: allocateMacrosContract,
};
